	...
	public class EvernoteAPICommand extends Command
	{
		...
		override public function execute():void
		{
			...
			*/
			...
			if ( event.type == EvernoteAPICommandTriggerEvent.STRING ) 
			{
				this.service.string( event.shareKey )
				this.service.eventDispatcher.addEventListener( EvernoteServiceEvent.STRING, this.stringResultHandler )
				this.service.eventDispatcher.addEventListener( EvernoteServiceEvent.STRING_FAULT, this.stringFaultHandler )
			}
...
*/
		...
		private function stringResultHandler(e:EvernoteServiceEvent)  : void
		{
			if ( seqId != this.service.getSequenceNumber()) return; 
			if ( this.event.fxSuccess != null ) this.event.fxSuccess(e.data);
			// 
		...
		private function stringFaultHandler(e:EvernoteServiceEvent)  : void
		{
			if ( seqId != this.service.getSequenceNumber()) return; 			
			if ( this.event.fxFault != null ) this.event.fxFault(e.data);
			this.onFault(); this.deReference()
		...
		private function deReference() : void
		{
			...
			if ( event.type == EvernoteAPICommandTriggerEvent.STRING ) 
			{
				this.service.eventDispatcher.removeEventListener( EvernoteServiceEvent.STRING, this.stringResultHandler )
				this.service.eventDispatcher.removeEventListener( EvernoteServiceEvent.STRING_FAULT, this.stringFaultHandler )
			}
			if ( event.type == EvernoteAPICommandTriggerEvent.EMAIL_NOTE ) 
			{
