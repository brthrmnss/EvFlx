...
{	
	...
	[SkinPart(required="true", type="fbrust.components.HighlightItemList")]
	...
	{
        _selectedItem = value;
        _previouslyEnteredText = text = returnFunction(_selectedItem);
	...
	override protected function partAdded(partName:String, instance:Object):void 
	{
		...
		{
			cancelButton.source = getStyle("iconCancel");
			cancelButton.addEventListener(MouseEvent.CLICK, cancel_clickHandler);
			cancelButton.addEventListener("iconCancelChanged", cancelButton_iconChangedHandler);			
		}
	...
	{            
		...
		if(cancelButton)
			cancelButton.visible = Boolean(text.length);	
	...
	private function filterFunction(item:Object):Boolean 
	{
		...
		*/
		var split : Array = text.toLowerCase().split(', ')
		var searchTxt : String = split[split.length-1].toLowerCase()
		// prefix search mode
		if (searchMode == SearchMode.PREFIX_SEARCH) 
		{
			if (itemLabel.substr(0, text.length) == searchTxt)
				return true;
			else 
				return false;
		}
		...
		{
			if (itemLabel.search(searchTxt) != -1)
				return true;   
		}		
